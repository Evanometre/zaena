...
const supabaseAdmin = createClient(supabaseUrl, supabaseServiceKey);
    ...
    // ============================================================
    // STEP 2: Check duplicates (phone-first, name fallback)
    // ============================================================

    const { data: existingSuppliers, error: fetchError } = await supabaseAdmin
      .from('suppliers')
    ...
    (existingSuppliers || []).forEach(s => {
      if (s.phone) existingPhoneMap.set(s.phone.trim().toLowerCase(), { id: s.id, name: s.name });
      existingNameMap.set(s.name.trim().toLowerCase(), { id: s.id, phone: s.phone });
    });

    const duplicates: DuplicateSupplier[] = [];
    ...
    rows.forEach((row) => {
      ...
      if (isDuplicate && duplicateStrategy === 'overwrite') {
        ...
        rowsToUpdate.push({ ...supplier, id: existingId });
      } else {
        rowsToInsert.push(supplier);
      }
    });
    ...
    for (let i = 0; i < rowsToInsert.length; i += CHUNK_SIZE) {
      ...
      console.log(`Inserted chunk ${i / CHUNK_SIZE + 1}: ${chunk.length} suppliers`);
    }
    ...
    for (let i = 0; i < rowsToUpdate.length; i += CHUNK_SIZE) {
      ...
      console.log(`Updated chunk ${i / CHUNK_SIZE + 1}: ${chunk.length} suppliers`);
    }
