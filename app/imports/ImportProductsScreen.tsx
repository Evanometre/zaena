// app/imports/ImportProductsScreen.tsx
...
export default function ImportProductsScreen() {
  ...
  const handlePickFile = async () => {
    try {
      ...
      // Read the file
      const file = new File(Paths.cache, asset.name);
      // Copy asset to cache so we can read it
  ...
  const renderIdle = () => (
    <View style={styles.centerContent}>
      <View style={styles.uploadBox}>
        <Text style={styles.uploadIcon}>📥</Text>
        <Text style={styles.uploadTitle}>Import Products</Text>
        <Text style={styles.uploadSubtitle}>
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          ...
          <Text style={styles.modalSubtitle}>
            {duplicates.length} product{duplicates.length > 1 ? "s" : ""}{" "}
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              Alert.alert(
                ...
                `This will update ${duplicates.length} existing product${duplicates.length > 1 ? "s" : ""} with new data. This cannot be undone.`,
                [
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const example =
        "Widget A,WGT-001,Electronics,pcs,10.00,25.00\n" +
        "Widget B,WGT-002,Electronics,pcs,8.50,20.00\n" +
        "Coffee Mug,,Kitchenware,pcs,3.00,9.99";
