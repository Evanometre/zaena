// FILE: app/trust/summary-report.tsx
// Personal summary — score + all five dimensions + key metrics
// Simpler than institutional: no score history chart, no credit ceiling
...
import { useAuthStore } from "../../stores/authStore"; // adjust
...
// ─── PDF builder ──────────────────────────────────────────────────────────────
function buildSummaryHTML(r: SummaryData): string {
  const tc = TIER_COLORS[r.tier] ?? D.gold;
  const sc = scoreColor(r.overall_score);

  const dimRows = Object.entries(r.dimensions)
    .map(([k, v]) => {
      const c = scoreColor(v as number);
      return `<tr>
      <td>${DIMS[k]?.label ?? k} <span style="color:#B8A98C">(${DIMS[k]?.weight}%)</span></td>
      <td style="text-align:right;color:${c};font-weight:600">${(v as number).toFixed(1)}</td>
      <td style="width:100px;padding-left:10px">
        <div style="height:4px;background:#E8E0D0;border-radius:2px">
          <div style="width:${Math.min(100, v as number)}%;height:4px;background:${c};border-radius:2px"></div>
        </div>
      </td>
    </tr>`;
    })
...
<style>
  * { margin:0;padding:0;box-sizing:border-box }
  ...
  .dot { width:60px;height:60px;border-radius:50%;background:${tc};display:flex;
    align-items:center;justify-content:center;color:white;font-size:8px;
    letter-spacing:1px;font-family:monospace;text-align:center;line-height:1.5 }
  ...
  .score-row { display:flex;justify-content:space-between;align-items:center;
    padding:14px;background:#EDE7D9;border-radius:3px;margin-bottom:12px }
  .score-big { font-size:48px;font-weight:300;color:#1A1008;line-height:1 }
  .score-lbl { font-size:8px;color:#B8A98C;font-family:monospace;letter-spacing:2px }
  .score-sub { font-size:8px;color:#B8A98C;font-family:monospace;letter-spacing:1px;margin-top:4px }
  ...
  .footer { margin-top:8px;padding:14px;background:#EDE7D9;border-top:2px solid ${tc};border-radius:3px }
  .fb { font-size:8.5px;color:#7A6A52;font-family:monospace;letter-spacing:1.5px;margin-bottom:6px }
  .ft { font-size:8.5px;color:#B8A98C;font-family:monospace;line-height:1.7 }
  .fr { margin-top:8px;padding-top:8px;border-top:1px solid #D4C9B0;font-size:7.5px;color:#B8A98C;font-family:monospace;line-height:1.8 }
  ...
  <div class="ref-grid">
    ...
    <div class="ref-item" style="text-align:right"><label>REF</label><span>${r.report_ref}</span></div>
  </div>

  <div class="section">
    <div class="sh"><span class="sn">01</span><span class="sl">Score Overview</span></div>
    <div class="score-row">
      <div>
        <div class="score-lbl">ZAENA TRUST SCORE</div>
        <div class="score-big">${r.overall_score.toFixed(1)}</div>
        <div class="score-sub">OUT OF 100</div>
      </div>
      <div style="text-align:right">
        <div class="pill">${r.tier.replace("_", " ").toUpperCase()}</div><br/>
        <div class="rating">${scoreRating(r.overall_score)}</div>
        <div style="font-size:8px;color:#B8A98C;font-family:monospace;margin-top:4px">
          TRAJECTORY: ${TRAJ_LABEL[r.trajectory] ?? r.trajectory}
        </div>
      </div>
    </div>
    <table>
  ...
  <div class="section">
    <div class="sh"><span class="sn">02</span><span class="sl">Score Dimensions</span></div>
    <table>${dimRows}</table>
  </div>

  <div class="section">
    <div class="sh"><span class="sn">03</span><span class="sl">Key Metrics</span></div>
    <table>
      <tr><td>Total Transactions</td><td style="text-align:right">${r.metrics.total_sales}</td></tr>
      <tr><td>Operational Days</td><td style="text-align:right">${r.metrics.days_active}</td></tr>
      <tr><td>Cash Collection Rate</td><td style="text-align:right">${r.metrics.cash_collection_pct.toFixed(1)}%</td></tr>
      <tr><td>Credit Recovery Rate</td><td style="text-align:right">${r.metrics.recovery_rate_pct.toFixed(1)}%</td></tr>
      <tr><td>Avg. Recovery Time</td><td style="text-align:right">${r.metrics.avg_recovery_days.toFixed(0)} days</td></tr>
      <tr><td>Supplier Payment Rate</td><td style="text-align:right">${r.metrics.supplier_payment_pct.toFixed(1)}%</td></tr>
      <tr><td>Void Rate</td><td style="text-align:right">${r.metrics.void_rate_pct.toFixed(1)}%</td></tr>
      <tr><td>WhatsApp Receipt Rate</td><td style="text-align:right">${r.metrics.whatsapp_receipt_pct.toFixed(1)}%</td></tr>
    </table>
  </div>

  <div class="footer">
    <div class="fb">ZAENA TRUST INFRASTRUCTURE</div>
    <div class="ft">Generated from real-time operational data. For personal use only.</div>
    <div class="fr">REF: ${r.report_ref}<br/>VERIFY: zaena.app/verify/${r.report_ref}</div>
  </div>
</div></body></html>`;
}
...
export default function SummaryReportScreen() {
  ...
  const handleExport = useCallback(async () => {
    ...
    setExporting(true);
    try {
      const html = buildSummaryHTML(report);
      const { uri } = await Print.printToFileAsync({ html, base64: false });
      await Sharing.shareAsync(uri, {
        mimeType: "application/pdf",
        dialogTitle: `Summary Report — ${report.organization_name}`,
        UTI: "com.adobe.pdf",
      });
    } catch (e: any) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: D.paperDeep }}>
      ...
      >
        ...
        >
          <SectionHeader num="01" label="Score Overview" />
          <View
            style={{
              flexDirection: "row",
              alignItems: "center",
          ...
          >
            ...
            <View style={{ alignItems: "flex-end" }}>
              ...
              >
                {scoreRating(report.overall_score)}
              </Text>
              <Text
                style={{
                  fontFamily: "DM Mono",
              ...
              >
                {TRAJ_LABEL[report.trajectory] ?? report.trajectory}
              </Text>
            </View>
          </View>
          <DocRow
        ...
        >
          <SectionHeader num="02" label="Score Dimensions" />
          {(Object.entries(report.dimensions) as [string, number][]).map(
            ([k, v]) => (
        ...
        >
          <SectionHeader num="03" label="Key Metrics" />
          <DocRow
          ...
          <DocRow
            ...
            valueColor={scoreColor(report.metrics.cash_collection_pct)}
          />
          <DocRow
            label="Credit Recovery Rate"
            value={`${report.metrics.recovery_rate_pct.toFixed(1)}%`}
            valueColor={scoreColor(report.metrics.recovery_rate_pct)}
          />
          <DocRow
            label="Supplier Payment Rate"
            value={`${report.metrics.supplier_payment_pct.toFixed(1)}%`}
            valueColor={scoreColor(report.metrics.supplier_payment_pct)}
          />
          <DocRow
            label="Void Rate"
        ...
        >
          ...
          >
            Generated from real-time operational data. For personal use only.
          </Text>
          <View
            style={{
              marginTop: 8,
